...
Kodare-avkodar-arkitekturen som Manjavacas et al. (2019) föreslår tar ett löpord x<sub>t</sub> som indata och läser av det tecken för tecken. Syftet är att avkoda ett mållemma l<sub>t</sub> -- som är grundat i den mellanliggande representationen av $ x_{t} $ -- ett tecken i taget. För varje löpord $ x_{t} $ extraherar man en sekvens av teckeninbäddningar (eng. *token character embeddings*) *c<sup>x</sup><sub>1</sub>,...,c<sup>x</sup><sub>n</sub>* från en inbäddningsmatris *W<sub>enc</sub> ∈ R<sup>\|C\|×d</sup>*, där *\|C\|* är storleken hos teckenvokauläret och *d* inbäddningsdimensionaliteten.
